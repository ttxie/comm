...
static const u_char *md5_body(md5_t *ctx, const u_char *data,
                size_t size);
...
void md5_update(md5_t *ctx, const void *data, size_t size)
{
        ...
        if (size >= 64) {
                data = md5_body(ctx, (const u_char*)data, size & ~(size_t) 0x3f);
                size &= 0x3f;
        }
...
/*
   函数：md5()
   参数说明：
srcdata: 无符号字符串（u_char）,需要加密的字符串
result: 无符号字符串(u_char),加密后的字符串并且已转换为16进制
返回值：0表示对字符串成功加密并且转换为16进制
 */
void md5(u_char *srcdata, char *result)
{
        ...
        len = strlen((const char*)srcdata);
        md5_init(&md5Con);
        md5_update(&md5Con, srcdata, len);
        md5_final((u_char*)result, &md5Con);
...
#if (HAVE_LITTLE_ENDIAN && HAVE_NONALIGNED)
